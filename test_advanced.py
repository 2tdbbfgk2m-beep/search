from memory_layer import LLMController, AgenticMemorySystem, safe_output_path
import os
import io
import json
import argparse
...
def evaluate_dataset(dataset_path: str, model: str, output_path: Optional[str] = None, ratio: float = 1.0, backend: str = "sglang", temperature_c5: float = 0.5, retrieve_k: int = 10, sglang_host: str = "http://localhost", sglang_port: int = 30000):
    ...
    for sample_idx, sample in enumerate(samples):
        ...
        if os.path.exists(memory_cache_file):
            ...
            memories_cache_buffer = io.BytesIO()
            pickle.dump(memories_to_cache, memories_cache_buffer)
            Path(safe_output_path(memory_cache_file)).write_bytes(memories_cache_buffer.getvalue())
    ...
    if output_path:
        ...
        json.dump(final_results, results_buffer, indent=2)
        Path(safe_output_path(output_path)).write_text(results_buffer.getvalue())
