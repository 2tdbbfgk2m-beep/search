...
def safe_output_path(path) -> str:
    """Validate a caller-provided cache/output path before any file write.

    Resolves relative paths against the current working directory and
    refuses NUL bytes, absolute paths, and anything that would resolve
    outside the working directory. Read side is unaffected.
    """
    text = str(path)
    ...
    if candidate.is_absolute():
        raise ValueError(
            f"output paths must be relative to the working directory: {path!r}"
        )
...
class HybridRetriever:
    ...
    def save(self, retriever_cache_file: str, retriever_cache_embeddings_file: str):
        ...
        if self.embeddings is not None:
            np.save(safe_output_path(retriever_cache_embeddings_file), self.embeddings)
        ...
        pickle.dump(state, cache_buffer)
        Path(safe_output_path(retriever_cache_file)).write_bytes(cache_buffer.getvalue())
